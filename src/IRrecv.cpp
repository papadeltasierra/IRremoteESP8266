...
#if IRREMOTEESP8266_USE_RMT_RX && \
    defined(ESP_ARDUINO_VERSION_MAJOR) && (ESP_ARDUINO_VERSION_MAJOR >= 3)
#define IRREMOTEESP8266_RMT_RX_SUPPORTED
#include <driver/rmt_rx.h>
#endif // IRREMOTEESP8266_USE_RMT_RX
#endif
...
#ifdef IRREMOTEESP8266_RMT_RX_SUPPORTED
  static rmt_channel_handle_t rmt_rx_channel = NULL;
  static rmt_symbol_word_t *rmt_symbols = NULL;
  static size_t rmt_symbol_capacity = 0;
  static bool rmt_enabled = false;
#endif // IRREMOTEESP8266_RMT_RX_SUPPORTED
#endif // ESP32
...
/// @cond IGNORE
...
#if defined(IRREMOTEESP8266_RMT_RX_SUPPORTED) && !defined(UNIT_TEST)
  /// Store a completed RMT capture in the representation used by all decoders.
  static bool USE_IRAM_ATTR rmt_rx_done(
      rmt_channel_handle_t channel __attribute__((unused)),
      ...
      void *user_data __attribute__((unused)))
  {
    ...
    if (edata->num_symbols && params.bufsize)
    {
      // The GPIO backend stores a leading placeholder, followed by mark/space.
      params.rawbuf[rawlen++] = 1;
      for (size_t symbol = 0; symbol < edata->num_symbols; symbol++)
      {
        const rmt_symbol_word_t &item = edata->received_symbols[symbol];
        const uint16_t durations[] = {item.duration0, item.duration1};
        for (uint8_t half = 0; half < 2; half++)
        {
          ...
          params.rawbuf[rawlen++] = durations[half] / kRawTick;
        }
      }
    }
  done:
...
#endif // IRREMOTEESP8266_RMT_RX_SUPPORTED && !UNIT_TEST
...
/// @endcond
  ...
  void IRrecv::enableIRIn(const bool pullup)
  {
    ...
    config.gpio_num = static_cast<gpio_num_t>(params.recvpin);
    config.flags.invert_in = false;
    config.flags.with_dma = false;
    if (rmt_new_rx_channel(&config, &rmt_rx_channel) != ESP_OK)
      return;
    rmt_symbol_capacity = (params.bufsize + 1) / 2;
    rmt_symbols = new rmt_symbol_word_t[rmt_symbol_capacity];
    if (rmt_symbols == NULL)
    {
      rmt_del_channel(rmt_rx_channel);
      rmt_rx_channel = NULL;
      return;
    }
    rmt_rx_event_callbacks_t callbacks = {};
    callbacks.on_recv_done = rmt_rx_done;
    if (rmt_rx_register_event_callbacks(rmt_rx_channel, &callbacks, NULL) !=
        ESP_OK)
    {
      delete[] rmt_symbols;
      rmt_symbols = NULL;
...
#else // IRREMOTEESP8266_RMT_RX_SUPPORTED
...
#endif // ESP8266
    // Attach Interrupt
#if !defined(IRREMOTEESP8266_RMT_RX_SUPPORTED)
...
#if defined(ESP8266)
    os_timer_disarm(&timer);
#elif defined(IRREMOTEESP8266_RMT_RX_SUPPORTED)
  if (rmt_rx_channel != NULL)
  {
    if (rmt_enabled)
      rmt_disable(rmt_rx_channel);
    rmt_del_channel(rmt_rx_channel);
    rmt_rx_channel = NULL;
    rmt_enabled = false;
  }
...
#if defined(ESP32)
#ifdef IRREMOTEESP8266_RMT_RX_SUPPORTED
    if (rmt_rx_channel != NULL && rmt_enabled)
    {
      rmt_disable(rmt_rx_channel);
      rmt_enabled = false;
    }
...
#if defined(ESP32)
#ifdef IRREMOTEESP8266_RMT_RX_SUPPORTED
    if (rmt_rx_channel == NULL || rmt_symbols == NULL)
      return;
    if (!rmt_enabled && rmt_enable(rmt_rx_channel) != ESP_OK)
      return;
    rmt_enabled = true;
    rmt_receive_config_t config = {};
    config.signal_range_min_ns = 1000;
    config.signal_range_max_ns = MS_TO_USEC(params.timeout) * 1000U;
    if (rmt_receive(rmt_rx_channel, rmt_symbols,
                    rmt_symbol_capacity * sizeof(*rmt_symbols), &config) != ESP_OK)
      rmt_enabled = false;
#else // IRREMOTEESP8266_RMT_RX_SUPPORTED
                            ...
                            uint8_t tolerance, uint16_t delta)
  {
    ...
    return measured >= ticksLow(std::min(
                                    desired, static_cast<uint32_t>(MS_TO_USEC(params.timeout))),
                                tolerance,
                                delta);
  }
