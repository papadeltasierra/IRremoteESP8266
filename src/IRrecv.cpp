...
/// @cond IGNORE
  ...
  /// Interrupt handler for changes on the GPIO pin handling incoming IR
  /// messages.
  static void USE_IRAM_ATTR gpio_intr() {
    ...
    // @see
    // https://github.com/espressif/arduino-esp32/blob/6b0114366baf986c155e8173ab7c22bc0c5fcedc/cores/esp32/esp32-hal-timer.c#L106-L110
    ...
    // @see https://github.com/crankyoldgit/IRremoteESP8266/issues/1350
    // @see
    // https://github.com/espressif/arduino-esp32/blob/6b0114366baf986c155e8173ab7c22bc0c5fcedc/cores/esp32/esp32-hal-timer.c#L176-L178
...
///   or (0 to 1) (ESP32-C3)
  ...
  IRrecv::IRrecv(const uint16_t recvpin, const uint16_t bufsize,
    ...
    _timer_num = std::min(timer_num, (uint8_t)(
...
/// @endcond
    ...
    // If we have been asked to use a save buffer (for decoding), then create
    // one.
    if (save_buffer) {
  ...
  /// @param[in] pullup A flag indicating should the GPIO use the internal
  /// pullup resistor. (Default: `false`. i.e. No.)
  void IRrecv::enableIRIn(const bool pullup) {
    ...
    // Initialise state machine variables for all platforms.
    resume();
...
#elif defined(ESP32)
#ifdef ESP_GENERIC
  // Use generic timer-based reception instead of RMT.
  if (timer == NULL) {
    timer = timerBegin(0, 80, true);  // 80 MHz, 1 microsecond per tick
    timerAttachInterrupt(timer, &read_timeout, true);
  }
  // Attach GPIO interrupt for edge detection.
  attachInterrupt(params.recvpin, gpio_intr, CHANGE);
#else   // ESP_GENERIC
  // Use RMT-based reception (default for ESP32).
  IRrecvRMT_enableIRIn(params.recvpin, params.bufsize, params.timeout);
#endif  // ESP_GENERIC
#endif  // ESP32
#endif  // UNIT_TEST
  }
...
#elif defined(ESP32)
#ifdef ESP_GENERIC
  // Stop generic timer-based reception.
  if (timer != NULL) {
    timerEnd(timer);
    timer = NULL;
  }
  detachInterrupt(params.recvpin);
#else   // ESP_GENERIC
  // Stop RMT-based reception.
  IRrecvRMT_disableIRIn(params.recvpin);
#endif  // ESP_GENERIC
#endif  // ESP32
#endif  // UNIT_TEST
  }
...
#if defined(ESP32)
#ifdef ESP_GENERIC
    // Generic mode: pause is handled by state change above.
#else   // ESP_GENERIC
  IRrecvRMT_pause(params.recvpin);
#endif  // ESP_GENERIC
#endif  // ESP32
  }

  /// Resume collection of received IR data.
  /// @note This is required if `decode()` is successful and `save_buffer`
  ///   was not set when the class was instanciated.
  /// @see IRrecv class constructor
...
#if defined(ESP32)
#ifdef ESP_GENERIC
    // Generic mode: resume is handled by state change above.
#else   // ESP_GENERIC
  IRrecvRMT_resume(params.recvpin);
#endif  // ESP_GENERIC
#endif  // ESP32
  }

  /// Make a copy of the interrupt state & buffer data.
  /// Needed because irparams is marked as volatile, thus memcpy() isn't
  /// allowed. Only call this when you know the interrupt handlers won't modify
  /// anything. i.e. In kStopState.
  ...
  void IRrecv::copyIrParams(atomic_irparams_t * src, irparams_t * dst) {
    ...
    // This isn't immediately obvious due to typecasting/different variable
    // names.
...
#if DECODE_HASH
  /// Set the minimum length we will consider for reporting UNKNOWN message
  /// types.
  ...
  /// @note There is a trade-off here. Saving the state means less time lost
  /// until we can receiving the next message vs. using more RAM. Choose
  /// appropriately.
  ...
  /// @param[in] noise_floor Pulses below this size (in usecs) will be removed
  /// or
  ///   merged prior to any decoding. This is to try to remove noise/poor
  ///   readings & slightly increase the chances of a successful decode but at
  ///   the cost of data fidelity & integrity. (Defaults to 0 usecs. i.e. Don't
  ...
  ///   **Any** non-zero value has the potential to **cook** the captured raw
  ///   data i.e. The raw data is going to lie to you. It may obscure hardware,
  ///   circuit, & environment issues thus making it impossible to support you
  ///   accurately or confidently.
  ...
  bool IRrecv::decode(decode_results * results, irparams_t * save,
...
#if defined(ESP32)
#ifndef ESP_GENERIC
    // RMT mode: poll for new data.
    IRrecvRMT_poll(params.recvpin, params.rawbuf, params.bufsize, params.rawlen,
                   params.overflow, params.rcvstate, params.timeout);
#endif  // !ESP_GENERIC
        // Generic mode: gpio_intr and read_timeout handle data collection.
#endif  // ESP32
#ifndef UNIT_TEST
    ...
    // However, don't do this if rawbuf is already full as we stomp over the
    // heap. See: https://github.com/crankyoldgit/IRremoteESP8266/issues/1516
    ...
    for (uint16_t offset = kStartOffset;
         offset <= (max_skip * 2) + kStartOffset; offset += 2) {
      ...
      // similar in timings & structure, but the Sanyo one is much longer than
      // the NEC protocol (42 vs 32 bits) so this one should be tried first to
      ...
      // similar in timings & structure, but the Epson one is much longer than
      // the NEC protocol (3x32 identical bits vs 1x32 bits) so this one should
      // be tried first to try to reduce false detection as a NEC packet.
...
#endif
#if DECODE_GICABLE
      // Note: Needs to happen before JVC decode, because it looks similar
      // except
      ...
      // Bosch is similar to Coolix, so it must be attempted before
      // decodeCOOLIX.
      ...
      // Kelvinator based-devices use a similar code to Gree ones, to avoid
      // false matches this needs to happen before decodeGree().
      ...
      /* NOTE: Disabled due to poor quality.
        ...
        // *IF* you are going to enable it, do it near last to avoid false
    positive
        // matches.
      ...
      */
...
#endif
#if DECODE_GREE
      // Gree based-devices use a similar code to Kelvinator ones, to avoid
      // false matches this needs to happen after decodeKelvinator().
      ...
      if (decodeArgoWREM3(results, offset, kArgo3AcControlStateLength * 8,
                          true))
        return true;
      DPRINTLN("Attempting Argo WREM3 decode (iFeel report)");
      if (decodeArgoWREM3(results, offset, kArgo3iFeelReportStateLength * 8,
                          true))
        return true;
...
#if DECODE_TRUMA
      // Needs to happen before decodeMultibrackets() as they can appear
      // similar.
      ...
      if (decodeToto(results, offset,
                     kTotoLongBits) ||  // Long needs to be first
  ...
  uint32_t IRrecv::ticksLow(const uint32_t usecs, const uint8_t tolerance,
    ...
    return (static_cast<uint32_t>(std::max(
        static_cast<int32_t>(
            usecs * (1.0 - _validTolerance(tolerance) / 100.0) - delta),
  ...
  uint32_t IRrecv::ticksHigh(const uint32_t usecs, const uint8_t tolerance,
                             const uint16_t delta) {
    return (static_cast<uint32_t>(usecs *
                                  (1.0 + _validTolerance(tolerance) / 100.0)) +
            1 + delta);
  }
...
#ifdef UNIT_TEST
    // Sanity checks that we don't have values that cause integer
    // over/underflow. Only performed during testing so there is no performance
  ...
  bool IRrecv::matchAtLeast(uint32_t measured, uint32_t desired,
    ...
    DPRINT(" >= ");
    DPRINT(ticksLow(
        std::min(desired, static_cast<uint32_t>(MS_TO_USEC(params.timeout))),
        tolerance, delta));
    DPRINT(" [min(");
    DPRINT(ticksLow(desired, tolerance, delta));
    DPRINT(", ");
  ...
  /// range (in uSeconds) either side of the desired, after an expected is
  /// excess is added.
  /// @param[in] measured The recorded period of the signal pulse.
  /// @param[in] desired The expected period (in usecs) we are matching against.
  ...
  /// @param[in] onemark Nr. of uSeconds in an expected mark signal for a '1'
  /// bit.
  /// @param[in] onespace Nr. of uSecs in an expected space signal for a '1'
  /// bit.
  /// @param[in] zeromark Nr. of uSecs in an expected mark signal for a '0' bit.
  /// @param[in] zerospace Nr. of uSecs in an expected space signal for a '0'
  /// bit.
  /// @param[in] tolerance Percentage error margin to allow. (Default:
  /// kUseDefTol)
  /// @param[in] excess Nr. of uSeconds. (Def: kMarkExcess)
  /// @param[in] MSBfirst Bit order to save the data in. (Def: true)
  ///   true is Most Significant Bit First Order, false is Least Significant
  ///   First
  ...
  match_result_t IRrecv::matchData(
      ...
      const uint32_t onespace, const uint16_t zeromark,
      const uint32_t zerospace, const uint8_t tolerance, const int16_t excess,
  ...
  /// Match & decode the typical data section of an IR message.
  /// The bytes are stored at result_ptr. The first byte in the result equates
  /// to the first byte encountered, and so on.
  /// @param[in] data_ptr A pointer to where we are at in the capture buffer.
  /// @param[out] result_ptr A ptr to where to start storing the bytes we
  /// decoded.
  ...
  uint16_t IRrecv::matchBytes(atomic_uint16_t * data_ptr, uint8_t* result_ptr,
                              ...
                              const bool MSBfirst, const bool expectlastspace) {
    // Check if there is enough capture buffer to possibly have the desired
    // bytes.
    ...
    for (uint16_t byte_pos = 0; byte_pos < nbytes; byte_pos++) {
      ...
      match_result_t result =
          matchData(data_ptr + offset, 8, onemark, onespace, zeromark,
                    zerospace, tolerance, excess, MSBfirst, lastspace);
  ...
  /// The data is stored in result_bits_ptr or result_bytes_ptr depending on
  /// flag `use_bits`.
  /// @note Values of 0 for hdrmark, hdrspace, footermark, or footerspace mean
  /// skip that requirement.
  ///
  /// @param[in] data_ptr A pointer to where we are at in the capture buffer.
  /// @param[out] result_bits_ptr A pointer to where to start storing the bits
  /// we
  ///    decoded.
  ...
  uint16_t IRrecv::_matchGeneric(
      atomic_uint16_t * data_ptr, uint64_t* result_bits_ptr,
      uint8_t* result_bytes_ptr, const bool use_bits, const uint16_t remaining,
      const uint16_t nbits, const uint16_t hdrmark, const uint32_t hdrspace,
      const uint16_t onemark, const uint32_t onespace, const uint16_t zeromark,
      const uint32_t zerospace, const uint16_t footermark,
      const uint32_t footerspace, const bool atleast, const uint8_t tolerance,
    ...
    // Don't need to extend for footerspace because it could be the end of
    // message
    ...
    // Header
    if (hdrmark &&
        !matchMark(*(data_ptr + offset++), hdrmark, tolerance, excess))
      return 0;
    if (hdrspace &&
        !matchSpace(*(data_ptr + offset++), hdrspace, tolerance, excess))
      return 0;

    // Data
    if (use_bits) {  // Bits.
      match_result_t result = IRrecv::matchData(
          data_ptr + offset, nbits, onemark, onespace, zeromark, zerospace,
          tolerance, excess, MSBfirst, kexpectspace);
      if (!result.success) return 0;
      *result_bits_ptr = result.data;
      offset += result.used;
    } else {  // bytes
      uint16_t data_used = IRrecv::matchBytes(
          data_ptr + offset, result_bytes_ptr, remaining - offset, nbits / 8,
          onemark, onespace, zeromark, zerospace, tolerance, excess, MSBfirst,
          kexpectspace);
    ...
    if (footermark &&
      ...
      return 0;
    // If we have something still to match & haven't reached the end of the
    // buffer
  ...
  ///
  /// @param[in] data_ptr: A pointer to where we are at in the capture buffer.
  /// @param[out] result_ptr A ptr to where to start storing the bits we
  /// decoded.
  /// @param[in] remaining The size of the capture buffer remaining.
  /// @param[in] nbits Nr. of data bits we expect.
  /// @param[in] hdrmark Nr. of uSeconds for the expected header mark signal.
  ...
  uint16_t IRrecv::matchGeneric(
      atomic_uint16_t * data_ptr, uint64_t* result_ptr,
      const uint16_t remaining, const uint16_t nbits, const uint16_t hdrmark,
      const uint32_t hdrspace, const uint16_t onemark, const uint32_t onespace,
      const uint16_t zeromark, const uint32_t zerospace,
      const uint16_t footermark, const uint32_t footerspace, const bool atleast,
    ...
    return _matchGeneric(data_ptr, result_ptr, NULL, true, remaining, nbits,
                         ...
                         zerospace, footermark, footerspace, atleast, tolerance,
                         excess, MSBfirst);
  }
  ...
  /// @return If successful, how many buffer entries were used. Otherwise 0.
  uint16_t IRrecv::matchGeneric(
      atomic_uint16_t * data_ptr, uint8_t* result_ptr, const uint16_t remaining,
      const uint16_t nbits, const uint16_t hdrmark, const uint32_t hdrspace,
      const uint16_t onemark, const uint32_t onespace, const uint16_t zeromark,
      const uint32_t zerospace, const uint16_t footermark,
      const uint32_t footerspace, const bool atleast, const uint8_t tolerance,
  ...
  uint16_t IRrecv::matchGenericConstBitTime(
      atomic_uint16_t * data_ptr, uint64_t* result_ptr,
      const uint16_t remaining, const uint16_t nbits, const uint16_t hdrmark,
      const uint32_t hdrspace, const uint16_t one, const uint32_t zero,
      const uint16_t footermark, const uint32_t footerspace, const bool atleast,
      const uint8_t tolerance, const int16_t excess, const bool MSBfirst) {
    uint16_t offset = 0;
    uint64_t result = 0;
    ...
    if (footermark)
      return _matchGeneric(data_ptr, result_ptr, NULL, true, remaining, nbits,
                           hdrmark, hdrspace, one, zero, zero, one, footermark,
                           footerspace, atleast, tolerance, excess, MSBfirst);
    // Overwise handle like normal, except for the last bit. and no footer.
    uint16_t bits =
        (nbits > 0) ? nbits - 1 : 0;  // Make sure we don't underflow.
  ...
  ///   true is Most Significant Bit First Order, false is Least Significant
  ///   First
  /// @param[in] GEThomas Use G.E. Thomas (true) or IEEE 802.3 (false)
  /// convention?
  /// @return If successful, how many buffer entries were used. Otherwise 0.
  ...
  uint16_t IRrecv::matchManchester(
      atomic_const_uint16_t * data_ptr, uint64_t* result_ptr,
      const uint16_t remaining, const uint16_t nbits, const uint16_t hdrmark,
      const uint32_t hdrspace, const uint16_t half_period,
      const uint16_t footermark, const uint32_t footerspace, const bool atleast,
      const uint8_t tolerance, const int16_t excess, const bool MSBfirst,
      const bool GEThomas) {
    uint16_t offset = 0;
    ...
    if (hdrspace) {
      entry = *(data_ptr + offset++);
      // Check to see if the header space has merged with a data space half
      // period
    ...
    // Data
    uint16_t used = matchManchesterData(
        data_ptr + offset, result_ptr, remaining - offset, nbits, half_period,
        bank, tolerance, excess, MSBfirst, GEThomas);
    if (!used) return 0;  // Data did match.
    offset += used;
    // Footer
    if (footermark &&
        !(matchMark(*(data_ptr + offset), footermark + half_period, tolerance,
                    excess) ||
          matchMark(*(data_ptr + offset), footermark, tolerance, excess)))
      return 0;
    offset++;
  ...
  uint16_t IRrecv::matchManchesterData(
      ...
      const uint16_t half_period, const uint16_t starting_balance,
      const uint8_t tolerance, const int16_t excess, const bool MSBfirst,
      const bool GEThomas) {
    ...
    // Flip the bit if we have a starting balance. ie. Carry over from the
    // header.
    ...
    // short + long + short = 2 bits (Add the previous bit again, then flip &
    // add) short + long + long + short = 3 bits (add prev, flip & add, flip &
    // add) We can't start with a long.
    //
  ...
  atomic_irparams_t* IRrecv::_getParamsPtr(void) { return &params; }
#endif  // UNIT_TEST
  // End of IRrecv class -------------------
