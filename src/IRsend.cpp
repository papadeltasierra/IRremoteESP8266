...
#endif
#include <algorithm>
#if defined(ESP32) && IRREMOTEESP8266_USE_RMT_TX && \
    defined(ESP_ARDUINO_VERSION_MAJOR) && (ESP_ARDUINO_VERSION_MAJOR >= 3)
#define IRREMOTEESP8266_RMT_TX_SUPPORTED
#include <driver/rmt_tx.h>
#endif // IRREMOTEESP8266_USE_RMT_TX
#ifdef UNIT_TEST
#include <cmath>
#endif
#ifdef SWIGLIB
                     ...
                     const uint16_t hz)
{
  ...
  enableIROut(hz);
#if defined(IRREMOTEESP8266_RMT_TX_SUPPORTED) && !defined(UNIT_TEST)
  const uint16_t max_rmt_duration = (1U << 15) - 1;
  size_t segment_count = 0;
  for (uint16_t index = 0; index < len; index++)
    if (buf[index])
      segment_count += (buf[index] + max_rmt_duration - 1) / max_rmt_duration;
  const size_t symbol_count = (segment_count + 1) / 2;
  rmt_symbol_word_t *symbols = new rmt_symbol_word_t[symbol_count];
  if (symbols != NULL)
  {
    size_t segment = 0;
    for (uint16_t index = 0; index < len; index++)
    {
      uint32_t duration = buf[index];
      while (duration)
      {
        const uint16_t part = std::min(duration, (uint32_t)max_rmt_duration);
        rmt_symbol_word_t &symbol = symbols[segment / 2];
        if (segment & 1)
        {
          symbol.duration1 = part;
          symbol.level1 = index & 1;
        }
        else
        {
          symbol.duration0 = part;
          symbol.level0 = index & 1;
        }
        segment++;
        duration -= part;
      }
    ...
    if (segment & 1)
    {
      symbols[segment / 2].duration1 = 1;
      symbols[segment / 2].level1 = !symbols[segment / 2].level0;
    }
    rmt_tx_channel_config_t tx_config = {};
    tx_config.clk_src = RMT_CLK_SRC_DEFAULT;
    tx_config.gpio_num = static_cast<gpio_num_t>(IRpin);
    tx_config.mem_block_symbols = 64;
    tx_config.resolution_hz = 1000000;
    tx_config.trans_queue_depth = 1;
    tx_config.flags.invert_out = outputOn == LOW;
    tx_config.flags.with_dma = false;
    rmt_channel_handle_t tx_channel = NULL;
    rmt_encoder_handle_t encoder = NULL;
    rmt_copy_encoder_config_t encoder_config = {};
    uint32_t frequency = hz < 1000 ? hz * 1000UL : hz;
    rmt_carrier_config_t carrier_config = {};
    carrier_config.frequency_hz = frequency;
    carrier_config.duty_cycle = static_cast<float>(_dutycycle) / kDutyMax;
    ...
    if (rmt_new_tx_channel(&tx_config, &tx_channel) == ESP_OK &&
        ...
        (!modulation || _dutycycle >= kDutyMax ||
         rmt_apply_carrier(tx_channel, &carrier_config) == ESP_OK) &&
        rmt_enable(tx_channel) == ESP_OK)
    {
      rmt_transmit_config_t transmit_config = {};
      transmit_config.eot_level = 0;
      if (rmt_transmit(tx_channel, encoder, symbols,
                       symbol_count * sizeof(*symbols), &transmit_config) == ESP_OK &&
          rmt_tx_wait_all_done(tx_channel, -1) == ESP_OK)
      {
...
#endif // IRREMOTEESP8266_RMT_TX_SUPPORTED && !UNIT_TEST
