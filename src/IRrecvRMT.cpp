...
static void ResetRingBuffer(RMTRecvState* state) {
  if (state == nullptr || state->ringbuf == nullptr) return;
  size_t item_size;
  void* item = nullptr;
...
static void StartRmtReception(RMTRecvState* state) {
  if (state == nullptr || state->ringbuf == nullptr || state->capturing) return;
...
static void ParseRmtItemsIntoRawbuf(RMTRecvState* state, uint16_t* rawbuf,
  ...
  for (size_t i = 0; i < item_count; ++i) {
    ...
    for (size_t part = 0; part < 2; ++part) {
      ...
      if (!have_mark) {
        if (!is_mark) continue;  // Skip leading idle space.
        have_mark = true;
        rawlen = 1;
        rawbuf[0] =
...
void IRrecvRMT_poll(uint16_t pin, uint16_t* rawbuf, uint16_t bufsize,
  ...
  RMTRecvState* state = FindState(pin);
  if (state == nullptr || rawbuf == nullptr) return;
  if (state->ringbuf == nullptr) return;
  if (rcvstate == kStopState) return;
